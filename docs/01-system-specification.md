...
- **Branch naming:** `minicoder/<feature-request-id>` (e.g., `minicoder/FR-002`); one branch per
  feature, owned by the active Coder run. **Known discrepancy (predates issue #35, not yet
  reconciled):** the actual runtime implementation (`branchNameFor()` in
  `@minicoder/adapters-coder`, and `MockGitHubClient`'s identical convention) names branches
  `minicoder/<featureRunId>` — the opaque `feature_runs.id` generated by `generateId()`, not the
  human-readable `FR-<n>` form this bullet and docs/00 §3.11 describe. Issue #35's PR-discovery
  feature (below) matches against the real runtime convention specifically so it works against
  actual production branches; reconciling the docs and code onto one convention is separate,
  unstarted future work.
- **Automated PR discovery (issue #35):** `GitHubClient.listPullRequestsForBranch(owner, repo,
branchName, state?)` lists PRs whose head matches a given branch, filtered by state (default
  `open`). `github-reconciliation`'s scheduled task uses it in a pre-pass
