...
describe('PostgresDbClient.transaction()', () => {
  ...
  it('marks client dead and releases connection when BEGIN fails', async () => {
    const beginError = new Error('connection reset');
    const faultyPg = makeMockClient((sql: string) => {
      if (sql === 'BEGIN') return Promise.reject(beginError);
      return Promise.resolve({ rows: [] });
    });
    ...
    // Connection state is unknown after BEGIN failure — client must be dead
    ...
    // Pool must be informed the connection is bad
    expect(releaseMock).toHaveBeenCalledWith(beginError);
  });
  ...
  it('close() does not call release() again after BEGIN failure', async () => {
    const faultyPg = makeMockClient((sql: string) => {
      if (sql === 'BEGIN') return Promise.reject(new Error('connection reset'));
      return Promise.resolve({ rows: [] });
    });
    const releaseMock = faultyPg.release as ReturnType<typeof vi.fn>;
    const faultyClient = new PostgresDbClient(faultyPg);

    await faultyClient.transaction(async () => {}).catch(() => {});
...
describe('TxClient lifetime', () => {
  it('PostgresTxClient rejects query() after the transaction ends', async () => {
    const mockPg = makeMockClient();
    const client = new PostgresDbClient(mockPg);
  ...
  it('PostgresTxClient rejects execute() after the transaction ends', async () => {
    const mockPg = makeMockClient();
    const client = new PostgresDbClient(mockPg);
    let capturedTx!: TxClient;
    ...
    await expect(capturedTx.execute('INSERT INTO t VALUES (1)')).rejects.toThrow(/expired/i);
  });

  it('PostgresTxClient rejects operations after a rolled-back transaction', async () => {
    const mockPg = makeMockClient();
    const client = new PostgresDbClient(mockPg);
    let capturedTx!: TxClient;
