...
export interface ExportWorkflowEventsOptions {
  /**
   * Resume cursor — both fields together (or neither; passing only one is treated as unset).
   * PR #73 review fix (round 2, MEDIUM-2): resuming from `id > sinceEventId` alone is not safe on
   * its own, because not every `workflow_events.id` is generated by the time-sortable
   * `generateId()` scheme — `state repair --apply` inserts a row keyed by `crypto.randomUUID()`
   * instead, which has no consistent lexical ordering relative to `generateId()`'s ids. A plain
   * `id` cursor could therefore skip a later, ordinary event forever once a UUID-keyed row became
   * the cursor. `sinceOccurredAt`/`sinceEventId` together form a real keyset-pagination cursor:
   * `occurred_at > sinceOccurredAt OR (occurred_at = sinceOccurredAt AND id > sinceEventId)` —
   * `sinceEventId` here is purely the tiebreaker for same-timestamp rows, not the primary ordering
   * key. Omit both to export the oldest still-eligible batch unconditionally (see
...
export interface ExportWorkflowEventsResult {
  ...
  /** Paired with `lastEventId` to form the next call's `sinceOccurredAt`/`sinceEventId` cursor. */
...
/**
 * No-ops (`{attempted: false, exportedCount: 0, lastEventId: null, lastOccurredAt: null}`) unless
...
export async function exportWorkflowEventsToOtlp(
  ...
  if (!endpoint || endpoint.trim().length === 0) {
    return { attempted: false, exportedCount: 0, lastEventId: null, lastOccurredAt: null };
  }
  ...
  // Both halves of the cursor are required together — a caller passing only one (a malformed
  // resume) is treated as "no cursor," matching this option's own doc comment.
  const hasCursor = Boolean(opts.sinceOccurredAt && opts.sinceEventId);
  ...
  if (hasCursor) {
    // Composite keyset cursor (occurred_at, id) — see ExportWorkflowEventsOptions' doc comment
    // for why occurred_at, not id, is the primary ordering key.
    sql += ` AND (occurred_at > ? OR (occurred_at = ? AND id > ?))`;
    params.push(opts.sinceOccurredAt, opts.sinceOccurredAt, opts.sinceEventId);
  }
  ...
  if (events.length === 0) {
    return {
      attempted: true,
      exportedCount: 0,
      lastEventId: hasCursor ? (opts.sinceEventId ?? null) : null,
      lastOccurredAt: hasCursor ? (opts.sinceOccurredAt ?? null) : null,
    };
  }
