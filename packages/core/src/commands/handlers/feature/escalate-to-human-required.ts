...
export class EscalateToHumanHandler implements CommandHandler<EscalateToHumanPayload, FeatureExecutionState> {
  ...
  async execute(envelope: CommandEnvelope<EscalateToHumanPayload>, db: DbClient): Promise<CommandResult<FeatureExecutionState>> {
    ...
    return db.transaction(async (tx) => {
      ...
      const now = isoNow();
      await tx.execute(`UPDATE feature_runs SET current_execution_state = ?, version = ?, updated_at = ? WHERE id = ? AND version = ?`, [FeatureExecutionState.HUMAN_REQUIRED, nextVersion(run.version), now, featureRunId, expectedVersion]);
      const eventId = await writeWorkflowEvent(tx, { featureRunId, projectId, eventType: 'feature.human_required', fromState, toState: FeatureExecutionState.HUMAN_REQUIRED, actorId: envelope.actor.id, correlationId: envelope.correlationId });
      await writeOutboxEvent(tx, { eventType: 'feature.human_required', payload: { featureRunId, projectId, reason } });
      const result: CommandResult<FeatureExecutionState> = { commandId: envelope.commandId, accepted: true, resultingState: FeatureExecutionState.HUMAN_REQUIRED, emittedEventIds: [eventId] };
