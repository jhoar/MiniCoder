...
// featureRunId/projectId are generated by commands/helpers.ts's generateId() (`${Date.now()}-
// ${random}`), never a UUID — `.uuid()` would reject every real payload RecordCodePushedHandler
// emits. Fixed here (Phase 9 review) since this schema is directly in this phase's touched
// surface; the same latent mismatch on FeatureSelectedPayloadSchema/FeatureCodingStartedPayloadSchema
// above predates Phase 7 and is out of this fix's scope (see the FeatureCiRunningPayloadSchema
// comment below for that prior fix's own note).
