...
2. **Then, the sandbox's egress proxy is itself default-deny.** Set `SCM_ALLOWED_HOST` (a bare
   `host[:port]`, not a full URL — it's regex-anchor-matched against the proxied request's `Host`,
   per `infra/docker/coder-sandbox/egress-proxy/entrypoint.sh`) to your Gitea/GitLab instance's
   host in `.env`, then recreate the proxy so it picks up the new filter rule:
...
| Coder sandbox egress         | `SCM_ALLOWED_HOST`                                                            | Bare `host[:port]` (no scheme) of your Gitea/GitLab instance, added to the sandbox egress proxy's allow-list; not needed for GitHub (already baked in). Has no effect unless `CODER_SANDBOX_HTTPS_PROXY` is also set. See §3.1.3                                                                                                                                                             |
